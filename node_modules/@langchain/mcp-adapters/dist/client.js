...
export class MultiServerMCPClient {
    ...
    constructor(connections) {
        ...
        this.serverNameToTools = new Map();
        this.cleanupFunctions = [];
        this.transportInstances = new Map();
        if (connections) {
     ...
     */
    processEnvVarsInConfig(servers) {
     ...
     */
    processStdioConfig(serverName, config) {
     ...
     */
    processSSEConfig(serverName, config) {
        const sseConfig = {
            transport: 'sse',
            url: config.url,
        };
    ...
    async initializeStdioConnection(serverName, connection) {
        ...
        logger.debug(`Creating stdio transport for server "${serverName}" with command: ${command} ${args.join(' ')}`);
    ...
    async createSSETransport(serverName, url, headers, useNodeEventSource) {
        ...
        logger.debug(`Using browser EventSource for server "${serverName}". Headers may not be applied correctly.`);
        logger.debug(`For better headers support in browsers, consider using a custom SSE implementation.`);
        return new SSEClientTransport(new URL(url), {
            requestInit: { headers },
    ...
    async createNodeEventSourceTransport(serverName, url, headers) {
        ...
        try {
            ...
            logger.debug(`Extended EventSource not available, falling back to standard EventSource: ${extendedError}`);
            try {
                ...
                logger.warn(`Failed to load EventSource packages for server "${serverName}". Headers may not be applied to SSE connection: ${nodeError}`);
    ...
    async attemptReconnect(serverName, connection, maxAttempts = 3, delayMs = 1000) {
        ...
        while (!connected && (maxAttempts === undefined || attempts < maxAttempts)) {
            attempts++;
            logger.info(`Reconnection attempt ${attempts}${maxAttempts ? `/${maxAttempts}` : ''} for server "${serverName}"`);
            try {
                ...
                logger.error(`Failed to reconnect to server "${serverName}" (attempt ${attempts}): ${error}`);
            }
        }
        if (!connected) {
